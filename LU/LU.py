...
class LUDecompositionWithEliminationMatrix:
    ...
    def elimination_matrix(self, i, j, factor):
        ...
        :return: 소거 행렬
        """
        E = np.eye(self.size, dtype=object)  # np.eye -> 단위행렬 생성, dtype=object: Fraction 객체를 넣을 수 있음
        E[j, i] = -factor
        return E
    ...
    def is_integer_matrix(self):
        """U 행렬의 모든 값이 정수인지 확인"""
        for row in self.U:
            for elem in row:
                if not elem.denominator == 1:  # 실수 값이 있는지 확인 (분모가 1이 아니면 실수로 처리)
                    return False
        return True
    ...
    def decompose(self):
        ...
        for i in range(self.size):
            # 피벗 값이 0이면 행렬을 다시 생성하도록 함
            if self.U[i, i] == 0:
                raise ZeroDivisionError("Pivot element is zero, cannot perform LU decomposition.")
    ...
    def display_steps(self):
        """각 스텝의 소거 행렬, 역행렬, 갱신된 U 및 L 행렬을 출력"""
...
# 정수 LU 분해 반복 시도
def perform_integer_lu_decomposition(matrix_size=3, value_range=(-10, 10)):
    while True:
        try:
            # 가역 행렬 생성
            matrix_instance = InvertibleMatrix(matrix_size, value_range)
            ...
            # 실수 값이 없는 경우만 종료
            if lu_decomposition.is_integer_matrix():
                print("\n정수만 포함된 LU 분해 결과가 나왔습니다.")
            ...
            print(f"오류 발생: {e}. 새로운 행렬을 생성하여 다시 시도합니다.\n")

# 실행
perform_integer_lu_decomposition()
